...
list_dataitems(_Opts) ->
    Conf = hb_s3_config:load_s3_device_config(),
    Bucket = maps:get(bucket, Conf, <<"offchain-dataitems">>),
    Prefix = <<>>,  % No prefix - files are at root level
    
    ?event({s3_list_dataitems, {bucket, Bucket}, {prefix, Prefix}}),
    S3Config = hb_s3_config:load_s3_device_config(),
    case hb_s3_erlcloud:list_objects(Bucket, Prefix, #{}, S3Config) of
        {ok, Resp} ->
            Keys = extract_keys_from_list_response(Resp),
            ?event({s3_list_success, {objects, length(Keys)}}),
            DataitemIDs = extract_dataitem_ids_from_keys(Keys),
            {ok, DataitemIDs};
        {error, Reason} ->
...
extract_dataitem_ids_from_keys(Keys) ->
    lists:filtermap(
        fun(Key) ->
            % Files are at root level as "ID.ans104"
            case binary:split(Key, <<".ans104">>) of
                [ID, <<>>] -> {true, ID};
                _ -> false
            end
        end,
...
%% @doc Extract S3 object keys from hb_s3_erlcloud list response map
extract_keys_from_list_response(Resp) when is_map(Resp) ->
    maps:fold(
        fun(K, V, Acc) ->
            case (is_binary(K) andalso binary:split(K, <<"_">>, [global])) of
                [<<"object">>, _Idx, <<"key">>] -> [V | Acc];
                _ -> Acc
            end
        end,
        [],
